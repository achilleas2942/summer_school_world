...
namespace summer_school_world{
	...
	:nh_(nh), private_nh_(private_nh){
		...
		mavState_sub_ = nh_.subscribe("mavros/state", 1, 
							&VelNode::MavStateCallback, this, ros::TransportHints().tcpNoDelay());

        pub_timer_ = nh_.createTimer(ros::Duration(0.02), &VelNode::PubCallback, this,
			false);
        sts_timer_ = nh_.createTimer(ros::Duration(0), &VelNode::StsCallback, this, false);
        mavCom_timer_ = nh_.createTimer(ros::Duration(0), &VelNode::MavComCallback, this, false);
	...
	void VelNode::initParams(){
		private_nh_.getParam("actuator_enabled", actuator_enabled_);
		GetParameterArray(private_nh_, &position_controller_.Kp_p_, "control/Kp_p", Eigen::Vector3d(1.0, 1.0, 1.0));
		GetParameterArray(private_nh_, &position_controller_.Kv_p_, "control/Kv_p", Eigen::Vector3d(1.0, 1.0, 1.0));
		GetParameterArray(private_nh_, &position_controller_.Ki_p_, "control/Ki_p", Eigen::Vector3d(0.0, 0.0, 0.0));
	...
	void VelNode::PoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg){
	...
	void VelNode::OdomCallback(const nav_msgs::OdometryConstPtr& msg){
		EigenOdometry odometry;
	...
	void VelNode::VeloCallback(const geometry_msgs::TwistStampedConstPtr& msg){
		Eigen::Vector4d vel_yaw_rate;
	...
	void VelNode::PubCallback(const ros::TimerEvent &e){
		if (actuator_enabled_){
			Eigen::VectorXd ref_rpms;
			position_controller_.getRPMs(&ref_rpms);
			mav_msgs::ActuatorsPtr actuator_msg(new mav_msgs::Actuators);
	...
	void VelNode::StsCallback(const ros::TimerEvent &e){
	...
	void VelNode::MavComCallback(const ros::TimerEvent &e){
		switch (node_state_) {
	...
	void VelNode::MavStateCallback(const mavros_msgs::State::ConstPtr &msg) { 
		current_state_ = *msg;
	}
...
int main(int argc, char** argv) {
  ...
  summer_school_world::VelNode vel_node(nh, private_nh);
